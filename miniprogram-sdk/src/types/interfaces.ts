...
import { IRawFrameData, IRawBodyFrameData, IRawFaceFrameData, IRawAudioFrameData, IRawEventFrameData } from '../../../src/types/frame-data';
...
export interface IDecoder {
  decode(
    files: IRawBodyFrameData[],
    onFrame: (file: any, frame: any, index: number) => void
  ): void;
  abort(): void;
...
export interface IDebugOverlay {
  setVideoInfo(info: { name: string; body_id: number; id: number }): void;
  setAudioInfo(info: { sf: number; ef: number; ad: Uint8Array }): void;
  setEventData(info: { sf: number; ef: number; event: any[] }): void;
}
