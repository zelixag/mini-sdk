    ...
    while (true) {
        ...
        if ('' == mimeType) break;
            ...
            if ("text/plain;charset=UTF-8" === mimeType) {
                ...
                buffer.offset += dataLen;
                // 小程序可能没有 Blob，这里降级处理或者需要 Blob Polyfill
                if (typeof Blob !== 'undefined') {
                    item = new Blob([dataSlice as ArrayBuffer], { type: mimeType });
                } else {
                    item = dataSlice; // 直接返回 ArrayBuffer
                }
            }
            result.push(item);
        }
...
export class IBRAnimationGeneratorCharInfo_NN {
    ...
    evalSkeletonFromMovable(movableJointTransforms: RigidTransform[]): RigidTransform[] {
        ...
        const movableSet2 = new Set(this.movableJoints);
        for (let i of this.jointEvalOrder) {
...
export function transformMJT(mjt: any[]): RigidTransform[] {
    ...
    for (let i = 0; i < mjt.length; i++) {
        ...
        if (item !== null && !Array.isArray(item) && typeof item === 'object' && 'translate' in item) {
            // {translate: number[], rotate: number[]} format from face-frame-aligner
            // rotate is [qx, qy, qz] (3 components) matching RigidTransform.deserialize binary convention
            vec = item.translate ?? [0, 0, 0];
            const rot = item.rotate ?? [];
            if (rot.length >= 4) {
                // Proto format extends binary RigidTransform.deserialize: binary stores [qx,qy,qz] and derives w.
                // Proto adds w as 4th component → [qx, qy, qz, qw] = [x, y, z, w].
                // Convert to RigidTransform convention [w, x, y, z].
                quat = [rot[3], rot[0], rot[1], rot[2]];
            } else if (rot.length === 3) {
                // 3-component quaternion [qx, qy, qz], derive w (same as RigidTransform.deserialize)
                const [qx, qy, qz] = rot;
                const sumSq = qx * qx + qy * qy + qz * qz;
                const qw = Math.sqrt(Math.max(1.0 - sumSq, 0.0));
                quat = [qw, qx, qy, qz];
            } else {
                quat = [1.0, 0.0, 0.0, 0.0];
            }
        } else if (item && typeof item.slice === 'function') {
            // flat numeric array [tx, ty, tz, w, qx, qy, qz]
            vec = item.slice(0, 3);
            quat = item.slice(3);
        } else {
            vec = [item[0], item[1], item[2]];
            quat = [item[3], item[4], item[5], item[6]];
        }

        // Validate quaternion
        const sumSqr = quat[0]*quat[0] + quat[1]*quat[1] + quat[2]*quat[2] + quat[3]*quat[3];
        if (!isFinite(sumSqr) || sumSqr < 1e-6) {
            quat = [1.0, 0.0, 0.0, 0.0];
        }

        result[i] = new RigidTransform(
            Array.from(vec) as [number, number, number],
            Array.from(quat) as [number, number, number, number]
        );
    }
    return result;
}
