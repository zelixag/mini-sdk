// @ts-nocheck
// Data interface for avatar rendering pipeline
// Ported from Web SDK src/utils/DataInterface.ts
// Adapted for WeChat MiniProgram compatibility
...
/**
 * Decode a null-terminated UTF-8 string from the buffer.
 * Uses TextDecoder when available, falls back to manual UTF-8 decoding for MiniProgram.
 */
function decodeStr(buffer: ArrayBuffer | DataViewWithCustomOffset): string {
    ...
    // TextDecoder may not be available in all MiniProgram environments
    ...
    while (i < stringBytes.length) {
        ...
        if (c < 0x80) {
            ...
            result += String.fromCharCode(((c & 0x1F) << 6) | (stringBytes[i + 1] & 0x3F));
            i += 2;
        } else if (c < 0xF0) {
            result += String.fromCharCode(((c & 0x0F) << 12) | ((stringBytes[i + 1] & 0x3F) << 6) | (stringBytes[i + 2] & 0x3F));
            i += 3;
        } else {
            // 4-byte UTF-8 (surrogate pair)
            const codePoint = ((c & 0x07) << 18) | ((stringBytes[i + 1] & 0x3F) << 12) | ((stringBytes[i + 2] & 0x3F) << 6) | (stringBytes[i + 3] & 0x3F);
...
const TensorDataTypeMap: Record<string, any> = {
    'B': Uint8Array,
    'b': Int8Array,
    'H': Uint16Array,
    'h': Int16Array,
...
// Float16Array is not available in MiniProgram environments, skip it
...
/**
 * Load archived items from the buffer.
 * In MiniProgram, Blob is not available. For non-octet-stream types that would
 * normally create a Blob, we store the raw ArrayBuffer slice instead.
 */
    ...
    while (true) {
        ...
        if ('' == mimeType) break;
            ...
            if ('text/plain;charset=UTF-8' === mimeType) {
                ...
                // Use TextDecoder with fallback
                if (typeof TextDecoder !== 'undefined') {
                    const decoder = new TextDecoder('utf-8');
                    item = decoder.decode(charView);
                } else {
                    // Manual decode
                    item = _decodeUtf8Bytes(charView);
                }
            }
                ...
                if (typeof TextDecoder !== 'undefined') {
                    const decoder = new TextDecoder('utf-8');
                    text = decoder.decode(charView);
                } else {
                    text = _decodeUtf8Bytes(charView);
                }
                item = JSON.parse(text);
            }
            else if ('application/octet-stream' === mimeType) item = Tensor.deserialize(buffer);
            else {
                // In MiniProgram, Blob is not available.
                // Store raw ArrayBuffer slice instead for compatibility.
                const dataLen = decodeLongInt(buffer);
...
/** Manual UTF-8 byte array to string decoder (fallback for environments without TextDecoder) */
function _decodeUtf8Bytes(bytes: Uint8Array): string {
    let result = '';
    let i = 0;
    while (i < bytes.length) {
        ...
        if (c < 0x80) {
            ...
            const codePoint = ((c & 0x07) << 18) | ((bytes[i + 1] & 0x3F) << 12) | ((bytes[i + 2] & 0x3F) << 6) | (bytes[i + 3] & 0x3F);
            const offset = codePoint - 0x10000;
            result += String.fromCharCode(0xD800 + (offset >> 10), 0xDC00 + (offset & 0x3FF));
            i += 4;
        }
